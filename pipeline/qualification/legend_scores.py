...
def calculate_attribute_scores(
    ...
    if not pd.isna(row.get("playoff_receiving_yards")):
        scores["playoff_receiving_yards"] = percentile_score(
            row["playoff_receiving_yards"],
            position_data["playoff_receiving_yards"],
            True,
        )
...
def process_players(
    df: pd.DataFrame, min_position_players: int = 3, curation_dir: Path = None
) -> pd.DataFrame:
    ...
    if curation_dir:
        ...
        if historical_file.exists():
            try:
                ...
                historical_legends = dict(
                    zip(
                        historical_df["player_id"],
                        historical_df["full_name"],
                        strict=False,
                    )
                )
                logger.info(
                    f"Loaded {len(historical_legends)} historical pre-1974 legends"
                )
            except Exception as e:
            ...
            try:
                ...
                if player_id in manual_legends:
                    ...
                    results.append(
                        {
                            "player_id": player_id,
                            "full_name": historical_legends[
                                player_id
                            ],  # Use name from historical file
                            "position": position,
                            "legend_score": 98,
    ...
    for player_id, manual_data in manual_legends.items():
        if player_id not in processed_ids:
            logger.info(
                f"Adding manually curated legend not found in dataset: {player_id}"
            )
            results.append(
                {
                    "player_id": player_id,
                    "full_name": manual_data.get(
                        "full_name", f"Unknown Player ({player_id})"
                    ),
...
def main(
    ...
    if min_games > 0:
        ...
        logger.info(
            f"After filtering for {min_games}+ games: {len(df_eligible)} players"
        )
    else:
        df_eligible = df.copy()
    ...
    if "is_manually_curated" in results_df.columns:
        ...
        if manual_count > 0:
            manual_scores = results_df[results_df["is_manually_curated"]][
                "legend_score"
            ]
            logger.info(
                f"  Manual score range: {manual_scores.min():.1f} - {manual_scores.max():.1f}"
            )
    ...
    for pos in results_df["position"].unique():
        ...
        logger.info(
            f"  {pos}: {top_player['full_name']} ({top_player['legend_score']:.1f})"
        )
