...
def _load_nflverse_datasets(
    nfl_module, logger: logging.Logger, full_build: bool
) -> dict:
    ...
    datasets["seasonal"] = _load_seasonal_data_safe(
        nfl_module, seasonal_years, "REG", logger
    )
    datasets["playoff"] = _load_seasonal_data_safe(
        nfl_module, seasonal_years, "POST", logger
    )
...
def _load_seasonal_data_safe(
    nfl_module, years: list, season_type: str, logger: logging.Logger
):
    ...
    try:
        ...
        if min(years) <= 1998:
            ...
            data = nfl_module.import_seasonal_data(
...
def build_comprehensive_index(
    logger: logging.Logger, output_path: Path, full_build: bool = False
) -> bool:
    ...
    try:
        ...
        enhanced_players = _merge_player_datasets(
            datasets, career_stats, playoff_stats, logger
        )
...
def _merge_player_datasets(
    datasets: dict, career_stats, playoff_stats, logger: logging.Logger
):
    ...
    combine_subset = combine_subset[combine_subset["pfr_id"].notna()].drop_duplicates(
        subset=["pfr_id"]
    )
    ...
    return pd.DataFrame(
        {
            ...
            # Defensive stats
            "def_solo_tackles": enhanced_players["def_solo_tackles"]
            .fillna(0)
            .astype(int),
...
def _filter_players(
    final_index, enhanced_players, full_build: bool, logger: logging.Logger
):
...
def _save_and_report_results(final_index, output_path: Path, logger: logging.Logger):
    ...
    logger.info(
        f"Comprehensive player index saved: {len(final_index)} players → {output_path}"
    )
    ...
    for _, player in final_index.head(5).iterrows():
        height_str = (
            f'{player["height_in"]}"' if pd.notna(player["height_in"]) else "N/A"
        )
        weight_str = (
            f"{player['weight_lb']}lb" if pd.notna(player["weight_lb"]) else "N/A"
        )
        forty_str = (
            f"{player['forty_time']}s" if pd.notna(player["forty_time"]) else "N/A"
        )
...
def _generate_summary_report(final_index, logger: logging.Logger):
    ...
    logger.info(
        f"  With >10 games: {len(final_index[final_index['total_career_games'] > 10]):,}"
    )
    logger.info(f"  With Pro Bowls: {len(final_index[final_index['pro_bowls'] > 0]):,}")
    logger.info(
        f"  With offensive TDs: {len(final_index[final_index['career_tds'] > 0]):,}"
    )
    logger.info(
        f"  With playoff experience: {len(final_index[final_index['playoff_games'] > 0]):,}"
    )
    logger.info(
        f"  With playoff TDs: {len(final_index[final_index['playoff_tds'] > 0]):,}"
    )
    ...
    logger.info(
        f"  With combine data: {len(final_index[pd.notna(final_index['forty_time'])]):,}"
    )
    logger.info(
        f"  Hall of Fame: {len(final_index[final_index['hof_flag'] == True]):,}"
    )
