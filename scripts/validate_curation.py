    ...
    for i, legend in enumerate(yaml_data["legends"]):
        ...
        if "legend_score" in legend:
            ...
            if not isinstance(score, int) or score < 98 or score > 100:
                errors.append(
                    f"{legend_id}: legend_score must be integer 98-100, got {score}"
                )
    ...
    for filename, data in legends_data.items():
        for i, legend in enumerate(data.get("legends", [])):
            ...
            if player_id not in valid_player_ids:
                errors.append(
                    f"{filename}:legends[{i}]: Unknown player_id '{player_id}'"
                )
...
def validate_target_counts(legends_data: dict) -> list[str]:
    ...
    for filename, data in legends_data.items():
        ...
        if expected_count and target_count != expected_count:
            errors.append(
                f"{filename}: target_count is {target_count}, expected {expected_count} for position {position}"
            )

        if target_count and actual_count != target_count:
            errors.append(
                f"{filename}: Has {actual_count} legends, target_count is {target_count}"
            )
...
def main(player_data: str, curation_dir: str, verbose: bool):
    ...
    if all_errors:
        ...
        # Summary statistics
        total_legends = sum(
            len(data.get("legends", [])) for data in legends_data.values()
        )
